...
void Server::start() {
    ...
    acceptChannel_->setReadCallback(std::bind(&Server::connectionCallback, this));
    acceptChannel_->setEventNoUpdate(EPOLLIN | EPOLLET);
    loop_->addChannel(acceptChannel_);
    started_ = true;
    LOG << "WebServer started on port " <<port_;
}

void Server::connectionCallback() {
    ...
    while ((connfd = accept(listenFd_, (struct sockaddr*)&cliAddr, &len)) > 0) {
        ...
        setNodelay(connfd);
        std::shared_ptr<Channel> channel(new Channel(loop, connfd));
        loop->queueInLoop(std::bind(&Channel::newCallback, channel));
    }
